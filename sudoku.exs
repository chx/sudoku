...
defmodule Sudoku do
  ...
  defp assign({s, d}, values) when d in @digits do
    values[s] -- [d]
    |> Enum.reduce(values, &(eliminate(s, &1, &2)))
  end
...
Sudoku.test
Sudoku.test '003020600900305001001806400008102900700000008006708200002609500800203009005010300'

Sudoku.test '4.....8.5.3..........7......2.....6.....8.4......1.......6.3.7.5..2.....1.4......'
